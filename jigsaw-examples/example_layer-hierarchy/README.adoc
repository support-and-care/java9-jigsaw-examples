...
[NOTE]
====
This demonstrates an important aspect of Java Modules: _layer isolation at runtime_ provides stronger encapsulation than what's possible at compile time with automatic modules.
...
Both approaches redirect the hardcoded `mlib` path to Maven-built JARs without modifying shared source code.
====
