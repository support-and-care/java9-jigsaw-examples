...
*Dynamic module loading symlink:*
...
Since Maven (4) creates JARs in the `target/` directory (not `mlib/`), but the shared source code still references `mlib`, the migration uses a committed symbolic link:
...
[NOTE]
====
Both source symlinks (`m4/src/*/main/java → ../../../../src/*`) and the runtime symlink (`m4/mlib → target`) are committed to Git.
