...
This example contains `cpb`, which is classpath code (unnamed module) without module-info.java.
+
...
+
. Manual javac compilation of cpb to cplib/cpb.jar
. Maven compiles explicit modules (modb, modmain) with compiler arguments:
+
...
+
. Runtime uses `--module-path mlib` for modules and `--class-path cplib/cpb.jar` for classpath code
