...
echo "Should work without problems"
if ! "${JAVA_HOME}/bin/java" --add-opens=java.desktop/sun.awt=ALL-UNNAMED --module-path target -cp cplib/cpmain.jar pkgcpmain.MainCallingJavaDesktop 2>&1 | normalize | tee -a "${result_dir}/run.txt" | myecho; then
