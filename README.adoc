...
** *For cross-compilation example*: Java 8 (only needed for `example_compile-target-jdk8`)
...
The `example_spring-hibernate` may need updates for optimal Java 17+ compatibility.
====
...
| JDK | 17 LTS or later | Gradle example (`example_gradle-project`) | Required by Gradle 9.x | https://adoptium.net/ or https://sdkman.io/
| JDK | 8 LTS | only needed for `example_compile-target-jdk8` | For cross-compilation testing | https://adoptium.net/temurin/releases/?version=8
